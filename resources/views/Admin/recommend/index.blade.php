    ...
    <tr>
     ...
     <tr>
      <td width="350" valign="top">
       <form action="{{ url('/admin/recommend/insert') }}" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
      ...
      <td valign="top">
       <table width="100%" border="0" cellpadding="8" cellspacing="0" class="tableOnebor">
        ...
        @foreach($data as $item)
                <tr>
         <td><img src="/upload/{{ $item->img }}" width="100" /></a></td>
         <td>{{ $item->name }}</td>
         ...
         <td align="center"><a href="{{url('admin/recommend/edit')}}/{{$item->id}}">编辑</a> | <a href="{{url('admin/recommend/delete')}}/{{$item->id}}">删除</a></td>
...
//            获取当前文本框的值
              var v = $(obj).val();
//            $.post('路由','参数','执行后的返回结果')
            $.post('{{ url('/admin/recommend/changeorder') }}',{'_token':"{{ csrf_token() }}",'id':id,'cate_order':v},function(data){
//                console.log(data);
                if(data.status == 0){
